...
class DataOutsourceForm extends Component{
     constructor(props){
         super(props);
         this.state={
             data:[],
    ...
    render(){
        ...
        const description1style={
         ...
         >
          ...
          <Typography variant="h6">請選擇委外加工廠商 :</Typography>
          </ThemeProvider>
          </Box>
          <Box pl={1} mt={-2}>
          <Autocomplete
                  inputValue={this.state.outsource}
                  onChange={(event,newValue)=>{
                    if(this.state.showadd ==true || this.state.showEdit ==true){
                    if(newValue){
                          this.setState({
                            outsource:newValue.outsource
                          })
                      }
                  }}}
                  onInputChange={(e)=>{
                    if(this.state.showadd ==true || this.state.showEdit ==true){
                        this.setState({
                            outsource:e.target.value
                   })
                    }
                  }}
                  options={this.state.listoutsource}
                   
                  getOptionLabel={(option) => option.outsource}
                  style={{ width: 150 }}
                  disableClearable
                  renderInput={(params) => (
                    <TextField 
         ...
         >
             <Box pl={3}>
              <ThemeProvider theme={theme}>
          <Typography variant="h6">請選擇工單單號 :</Typography>
          </ThemeProvider>
          </Box>
          <Box pl={1} mt={-2}>
          <Autocomplete
         ...
         >
            ...
            <Fragment>
            <TableContainer style={{maxHeight :  500,width:700 ,border: "5px solid rgba(224, 224, 224, 1)"}}>
                <Table
                 columns={Columns}
         ...
         >
               <Box pl={15}>
              <ThemeProvider theme={theme}>
          <Typography variant="h6">委外數量 :</Typography>
          </ThemeProvider>
          </Box>
          <Box pl={1}>
          <TextField
...
export default DataOutsourceForm